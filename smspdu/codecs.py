...
class GSM:
    ...
    def decode(cls, data: str, strip_padding: bool = False) -> str:
        ...
        septets = [int(reversed_bits[k:k+7], 2)
                   for k in range(len(reversed_bits)-7, -1, -7)]
        res = ''
        is_extended = False
    ...
    def encode(cls, data: str, with_padding: bool = False) -> str:
        ...
        for char in data:
            ...
            if char_index == cls.CHAR_EXT:
                raise ValueError(
                    f"Char \"{char}\" can not be encoded with the GSM 7-bit codec")
        ...
        res = '0' * (len(chars) % 8) + \
            ''.join([f'{char:07b}' for char in chars][::-1])
